...
class PublicUserApi(TestCase):
    ...
    def test_create_token_no_user(self):
        #Test that token will not created it user does not exist
        payload = {
            'email':'test@gmail.com',
            'password':'1234'
        }
...
class PrivateUserApiTests(TestCase):
    ...
    def test_retrieve_profile_success(self):
        #Test that retriving profile for logged in user
        res = self.client.get(ME_URL)
    ...
    def test_post_me_not_allowed(self):
        #Test that POST method are not allowed in the me url
        res = self.client.post(ME_URL,{})

        self.assertEqual(res.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
    
    def test_update_user_profile(self):
        #Test that update the user profile for authenticated user
        payload = {
            'name':'new Name',
            'password':'new123'
        }
